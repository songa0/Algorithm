...
class Solution {
    public int solution(int[][] board, int[] moves) {
        int answer = 0;
        //각 열에서 가장 위의 index를 저장해두는 array
        int[] lastIdx = new int[board[0].length];
        //뽑힌 인형을 두는 Stack
        Stack<Integer> itemStack = new Stack<Integer>();  
        
        //가장 위에 위치한 인형의 위치 구하기
        for(int i =0; i<board[0].length; i++){
            for(int j =board.length-1; j>=0; j--){
                if(board[j][i]>0) lastIdx[i] = j;
                else break;
            }    
        }
        
        int removeIdx = 0; // 뽑힐 인형의 열
        int removeItem = 0; // 뽑힌 인형
        int latestItem = 0; // 가장 최근에 뽑힌 인형
            
        for(int i =0; i<moves.length; i++){
            removeIdx = moves[i]-1;
            removeItem = board[lastIdx[removeIdx]][removeIdx];
            
            if(!itemStack.empty())  latestItem = itemStack.peek();
            
            //뽑은 인형과 마지막 인형이 같은지 검사
            if(latestItem == removeItem && removeItem!=0){
                answer+=2;
                itemStack.pop();
                board[lastIdx[removeIdx]][removeIdx] = 0; //뽑은 인형 초기화
                if(lastIdx[removeIdx]<board.length-1) lastIdx[removeIdx]++; //가장 위 인형의 위치 ++
            }else if(removeItem!=0){
                itemStack.add(removeItem);
                 board[lastIdx[removeIdx]][removeIdx] = 0; //뽑은 인형 초기화
                 if(lastIdx[removeIdx]<board.length-1) lastIdx[removeIdx]++; //가장 위 인형의 위치 ++
            }
